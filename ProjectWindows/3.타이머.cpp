...
{
	MessageBox(hWnd, TEXT("홀리쉿~~"), _T("그래~~"), MB_YESNOCANCEL);
	HDC hdc = GetDC(hWnd);
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam) // 만들수는 있지만 자신이 직접 부를수는 없음
{
	...
	case WM_CREATE:
	{
		//SetTimer(hWnd, 1, 1000, NULL); // ( 핸들러 , 아이디 , 몇초후 , ? )
	...
	case WM_TIMER:
	{
		switch (wParam)
		{
		case 1: // 현재 시간을 구해서 화면에 표시한다
			SYSTEMTIME st;
			GetLocalTime(&st);
			wsprintf(sTime, TEXT("%d : %d : %d"), st.wHour, st.wMinute, st.wSecond);
			InvalidateRect(hWnd, NULL, TRUE);
			break;
		case 2: // 현재 시간을 구해서 화면에 표시한다
			MessageBox(hWnd, TEXT("담배피러가자~"), _T("그래~~"), MB_YESNOCANCEL);
			MessageBeep(0);
	...
	case WM_PAINT:
	{
		...
		//Rectangle(hdc , x - 20 , y - 20 , x , y); //사각형
